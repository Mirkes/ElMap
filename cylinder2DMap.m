classdef cylinder2DMap < MapGeometry
    ...
    methods
        function map = cylinder2DMap(rows, cols)
	    % rows will be circular component, while cols will be linear (parallel to the cylinder axis)
            if nargin < 2
            ...
            N=rows*cols;
            a1=repmat(1:rows,cols,1);
            a2=repmat((1:cols)',1,rows);
	    r = [a1(:),a2(:)];
	    temp = [cos(2*pi*(1:rows)/rows);sin(2*pi*(1:rows)/rows)]';
	    map.internal = repmat(temp,cols,1);
	    ttt = repmat((1:cols)/cols,rows,1);
	    map.internal(:,3) = ttt(:);
            % Form array of links
            ...
            E=A(:,2:end);
            map.links = [B(:), C(:); D(:), E(:)];
	    map.links = [map.links;[A(end,1:end); A(1,:)]']
            % Form array of ribs
            B1=A(1:end-2,:);
            B2=A(2:end-1,:);
            B3=A(3:end,:);
	    ...
	    map.ribs = [map.ribs;[A(end-1,1:end); A(end,1:end); A(1,:)]'];
	    map.ribs = [map.ribs;[A(end,1:end); A(1,1:end); A(2,:)]'];
            % Form array of faces
	    ...
	    map.faces=[B1(:), B2(:), B3(:); C1(:), C2(:), C3(:)];
            %map.faces=[B1(:), B2(:), B3(:); C1(:), C2(:), C3(:); D1(:), D2(:), D3(:); E1(:), E2(:), E3(:)];
